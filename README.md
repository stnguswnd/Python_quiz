...
사용자는 메뉴를 통해 퀴즈를 풀고, 새로운 퀴즈를 추가하고, 등록된 퀴즈 목록을 확인하고, 최고 퍼센트를 조회할 수 있습니다. 또한 `state.json` 파일을 사용해 퀴즈 데이터, 최고 퍼센트, 게임 기록을 저장하도록 구현해 프로그램을 종료한 뒤 다시 실행해도 이전 상태가 유지되도록 구성했습니다.
...
- 기록에는 플레이 시각, 푼 문제 수, 정답 수, 사용한 힌트 수, 총점, 퍼센트가 포함됩니다. `show_best_percent()`는 최고 퍼센트 조회만 담당하고, 기록 생성은 `record_history()`, 실제 파일 저장은 `save_state()`가 담당하도록 나누었습니다.
...
5. `best_percent`는 `float` 또는 `None`으로 복원
